...
export default async function handler(args = {}, options = {}) {
  ...
  const path = 'message.getFriendHistory';
  ...
  const resp = await wsCall({ url, path, args: [user_id, seq_start, limit], requestId, timeoutMs });
  return { success: true, data: { request: { type: 'sdk', path, args: [user_id, seq_start, limit], requestId }, response: resp } };
}
