...
export default async function handler(args = {}, options = {}) {
  ...
  const path = 'message.getGroupHistory';
  ...
  const seq_start = Number.isFinite(Number(args.seq_start)) ? Number(args.seq_start) : 0;
  const limit = Number.isFinite(Number(args.limit)) ? Number(args.limit) : 20;
  const resp = await wsCall({ url, path, args: [group_id, seq_start, limit], requestId, timeoutMs });
  return { success: true, data: { request: { type: 'sdk', path, args: [group_id, seq_start, limit], requestId }, response: resp } };
}
